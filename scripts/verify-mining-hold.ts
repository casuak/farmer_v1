...
import { MiningModel,ORES } from "../components/game/mining";
...
function rig(kind:"copper"|"iron"|"crystal"="copper"){
  ...
  const hits:number[]=[],breaks:number[]=[];let seconds=0;
  ...
  const tick=(dt:number)=>{seconds+=dt;const event=model.update(dt,player,"pickaxe");if(event?.ok){hits.push(seconds);if(event.broken){breaks.push(seconds);hold.clear();}}const next=hold.next(model,player,"pickaxe");if(next)assert(model.begin(next.id,player,"pickaxe").ok);return event;};
  const run=(seconds:number)=>{for(let i=0;i<Math.ceil(seconds/.01);i++)tick(.01);};
  return {model,hold,player,hits,breaks,start,tick,run};
}

...
for(const p of [{x:-8,z:7},{x:-13.1,z:1.8},{x:-8,z:-8},{x:0,z:40},{x:48,z:0},{x:9,z:5},{x:9,z:1.2}])assert(!w.canWalk(p.x,p.z));
for(let x=5.5;x<12.8;x+=.1)assert(w.canWalk(x,0),"The full bridge remains walkable");
assert.equal(WALK_SPEED,6);assert.equal(SPRINT_SPEED,12);
const fenceRunner={x:-8,z:-7};moveWithCollisions(fenceRunner,0,-5,w.canWalk);assert(fenceRunner.z>-7.7,"Fast movement cannot cross a thin fence");
const riverRunner={x:6,z:5};moveWithCollisions(riverRunner,8,0,w.canWalk);assert(riverRunner.x<9,"Fast movement cannot cross water");
const bridgeRunner={x:5.5,z:0};moveWithCollisions(bridgeRunner,7,0,w.canWalk);assert(Math.abs(bridgeRunner.x-12.5)<.001);
...
farm.update(50);assert.equal(farm.get(-4,-4)!.crop!.age,0,"Dry crops wait for water");
...
const view=createFarmView(scene,shadow,w.tiles,w.clearTile);view.updateTile(farm.get(-4,-4)!);
...
assert.equal(farm.get(-4,-4)!.crop!.age,GROWTH_SECONDS);
...
assert.equal(farm.get(-4,-4)!.crop,null);assert.equal(farm.get(-4,-4)!.watered,false);assert(farm.get(-4,-4)!.tilled);
view.updateTile(farm.get(-4,-4)!);
...
for(let i=0;i<4;i++)view.updateTile(farm.get(-4,-4)!);

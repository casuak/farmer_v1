...
import { FarmModel,worldToTile,type TileInfo,type TileKind,type Point } from "./farming";
...
export function createGame(canvas:HTMLCanvasElement,onReady:()=>void,onStatus:(s:GameStatus)=>void,onError:(s:string)=>void,events:{hover:(info:TileInfo|null)=>void;action:(result:InventoryResult)=>void;shop:()=>void}):GameApi {
  ...
  const heldTools=createHeldTools(scene,farmer.hand,shadow);heldTools.select(bag.tool);
  ...
  let mouse:{x:number;y:number}|null=null,hovered:{coord:Point;override?:TileKind}|null=null,actionCooldown=0,actionTime=0,hoverClock=0;
  ...
  function publish(){
    ...
    heldTools.select(boat.aboard?null:bag.tool);statusClock=0;
  }
  ...
  function pickTile(){
    if(!mouse||paused){hovered=null;farmView.hover(null);events.hover(null);return;}
    ...
    let coord=worldToTile(ray.origin.add(ray.direction.scale(distance))),override:TileKind|undefined;
    ...
    const info=farm.inspect(coord,avatar.position,bag.tool??"hoe",override);
    if(info&&(boat.aboard||!bag.tool)){info.actionable=false;info.hint=boat.aboard?"WASD 驾船 · 靠岸按 E 下船":"先在物品栏选择农具";}
    hovered=info?{coord,override}:null;farmView.hover(info);events.hover(info);
  }
  function useTool(useFacing=false){
    if(paused||actionCooldown>0)return;
    if(boat.aboard){announce({ok:false,message:"先靠岸下船，再使用农具"});return;}
    const tool=bag.tool;if(!tool){announce({ok:false,message:"先在物品栏选择锄头、种子、浇水壶或镰刀"});return;}
    pickTile();if(!hovered&&!useFacing){announce({ok:false,message:"请点击身边的地块"});return;}
  ...
  const renderFrame=()=>{
    ...
    if(!paused){
      ...
      let dx=(right-up)*Math.SQRT1_2,dz=(right+up)*Math.SQRT1_2,moved=false;const length=Math.hypot(dx,dz);
      ...
      const footfall=farmer.animate(t,dt,moved,mode.running,boat.aboard,actionTime,settings.motion);
  ...
  return {
    ...
    dropItem(index){if(paused)return;const item=bag.drop(index);if(!item){announce({ok:false,message:"先选择要丢弃的物品"});return;}groundItems.add(item,avatar.position);audio.play("drop");announce({ok:true,message:`放下${ITEMS[item.id].name} ×${item.count} · 按 E 可重新拾取`});},

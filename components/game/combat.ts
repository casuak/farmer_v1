...
import { createBulletTrails } from "./gunfire";
...
export class CombatModel{
  ...
  readonly shots:Shot[]=Array.from({length:8},()=>({active:false,x:0,z:0,y:.85,dx:0,dz:0,remaining:0,travelled:0,generation:0}));
  ...
  constructor(private canWalk:(x:number,z:number,r?:number)=>boolean,private clearLine:(a:Point,b:Point)=>boolean,private villagers:Hittable[]=[]){
    for(const [x,z] of SLIME_SPAWNS){
      ...
      for(let i=0;i<100&&!spawn;i++){
        ...
        if(inForest(p.x,p.z)&&canWalk(p.x,p.z,.42))spawn=p;
      }
  ...
  private recoil(slime:Slime,dt:number){
    ...
    for(let i=0;i<steps;i++){
      ...
      if(!inForest(next.x,next.z)||!this.canWalk(next.x,next.z,.4)||!this.clearLine(slime,next)){slime.knockX=slime.knockZ=0;return;}
  ...
  attack(weapon:Weapon,player:Point&{y?:number},aim:Point,muzzle?:Point&{y:number}):{fired:boolean;hits:number;heading:number}{
  ...
  update(dt:number):number{
    ...
    for(const s of this.slimes){
      ...
      if(inForest(nx,nz)&&(nextDistance<3.5||nextDistance<homeDistance)&&this.canWalk(nx,nz,.4)){s.x=nx;s.z=nz;s.travel-=dt;}else s.travel=0;
      if(s.travel<=0)s.wait=.6+this.random()*1.6;
    }
    for(const shot of this.shots)if(shot.active){
      let left=Math.min(dt*BULLET_SPEED,shot.remaining);
...
export function createCombat(scene:Scene,shadow:ShadowGenerator,canWalk:(x:number,z:number,r?:number)=>boolean,clearLine:(a:Point,b:Point)=>boolean,villagers:Hittable[]=[],onVillagerHurt?:(v:Hittable)=>void){
  ...
  update(0,true);return {model,views,bullets,numbers,trails,update};
}

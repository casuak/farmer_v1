...
import { Sprout,Sun,Moon,Sunrise,Sunset,Settings2,CircleHelp,Maximize,Minimize,Minus,Plus,MapPin,MousePointer2,ScanLine,ChevronUp,ChevronDown,ChevronLeft,ChevronRight,RotateCcw,Footprints,Check,Info,Coins,Map,Ship,Wind,Hand,Gauge } from "lucide-react";
...
export default function FarmGame(){
  const canvas=useRef<HTMLCanvasElement>(null),api=useRef<GameApi|null>(null),miningGesture=useRef(false);
  ...
  const [status,setStatus]=useState<GameStatus>(()=>({...FARM_SPAWN,location:"松溪农场",moving:false,running:true,aboard:false,fps:0,bag:new InventoryModel().snapshot(),interaction:null,clock:new DayNightClock().snapshot(),bloomAvailable:true,fishing:new FishingModel().snapshot(),mining:{active:false,progress:0,target:null},boss:{visible:false,name:BOSS.name,hp:BOSS.maxHp,maxHp:BOSS.maxHp,phase:"idle",telegraphProgress:0,playerHp:5,playerMaxHp:5,playerInvulnerable:false},dodge:new DodgeModel().snapshot()}));
  ...
  return <main className={`farm phase-three ${period==="夜晚"?"is-night":""} ${fishingActive?"is-fishing":""} ${status.boss.visible?"has-boss":""} ${status.bag.backpack?"has-backpack":""}`} aria-label="松溪农场，3D 体素游戏">
    <canvas className="game-canvas" ref={canvas} tabIndex={0} aria-label="游戏场景。WASD 移动，Shift 切换 3 倍跑步，空格翻滚（2.2 秒冷却），F 使用工具，E 互动，滚轮缩放，1 至 9 选物品。左键操作高亮地块或朝鼠标攻击，5 为手枪，6 为长剑，7 为钓鱼竿，8 为矿镐；靠近森林彩色矿石，按住左键或F连续敲击，也可按住 E 自动装备矿镐开采；矿物弹出落地后按 E 拾取。按住F或水面蓄力，松开甩竿；力度越大甩得越远。水面出现激烈水花和叹号时按F提竿，按住左键或F控制绿条，Esc 收竿。"/>
    ...
    <div className="hud adventure-actions">
      ...
      <button className="interact-button glass" disabled={!status.interaction} data-mining-held={!!status.mining.holding}
        onPointerDown={e=>{if(e.button!==0||!e.isPrimary)return;miningGesture.current=false;if(status.interaction?.kind==="mine"&&api.current?.miningPress()){e.preventDefault();miningGesture.current=true;e.currentTarget.setPointerCapture(e.pointerId);}}}
        onPointerUp={e=>{api.current?.miningRelease();if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId);}}
        onPointerCancel={()=>api.current?.miningRelease()} onLostPointerCapture={()=>api.current?.miningRelease()}
        onClick={()=>{if(miningGesture.current){miningGesture.current=false;return;}api.current?.interact();}}><kbd>E</kbd><Hand size={15}/><span>{status.interaction?.label??"靠近物品、镇民或小船互动"}</span></button>
    </div>
    ...
    <Dialog open={panel!==null} onOpenChange={open=>{if(!open)setPanel(null);}}>
      <DialogContent className={`farm-dialog ${panel==="shop"?"shop-dialog":""}`}>
        ...
        {panel==="shop"?<ShopPanel bag={status.bag} onSelect={i=>api.current?.selectSlot(i)} onBuy={id=>api.current?.buyItem(id)} onSell={(i,all)=>api.current?.sellItem(i,all)} onBackpack={()=>api.current?.buyBackpack()}/>:panel==="map"?<WorldMap x={status.x} z={status.z} aboard={status.aboard}/>:panel==="help"?<div>
          ...
          <div className="phase-note"><strong>听见矿石里的回声</strong>初始第 8 格已有矿镐。沿农场北侧林道进入青苔森林，地图上的彩色菱形标出 12 处矿点：橙色铜矿敲 3 次、银蓝铁矿敲 4 次、紫色晶簇敲 5 次。走近后按住左键点矿石，或面向矿石按住F；按住 E / 触屏互动按钮也会自动装备矿镐连续开采。松开后收完当前一镐，Esc 或切工具立即停止。每次抬镐、下砸、回弹，命中时矿石开裂并飞溅火星。矿脉碎裂后矿物会先弹出、落地并轻轻弹跳，落稳后靠近按 E 拾取，再飞入物品栏；满包不会丢失地面矿物。矿物可在杂货店出售，矿镐丢失也能买回。矿脉约 2 分钟后恢复，离开矿点再来看看。</div>

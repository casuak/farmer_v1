...
export function ItemIcon({id,className=""}:{id:ItemId;className?:string}){
  const cls=`item-art item-${id} ${className}`;
  if(isFishId(id)){
...
type Props={bag:InventorySnapshot;inspector:ReactNode;onSelect:(index:number)=>void;onMove:(from:number,to:number)=>void;onDrop:(index:number)=>void;locked?:boolean};
type Drag={index:number;item:Stack;x:number;y:number};
export default function InventoryUI({bag,inspector,onSelect,onMove,onDrop,locked=false}:Props){
  ...
  return <>
    ...
    <aside className="hud inventory-dock" aria-label="随身物品栏">
      <div className="selected-item-note"><MousePointer2 size={13}/><strong>{locked?"正在钓鱼":current?ITEMS[current.id].name:"空物品格"}</strong><span>{locked?"Esc 收竿 · 收回钓竿":moveFrom!==null?"点击目标格移动物品 · Esc 取消":drag?"拖到另一格整理 · 拖到场景放下":current?ITEMS[current.id].description:"拖动整理 · 1–9 快捷选择"}</span></div>

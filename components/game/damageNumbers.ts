...
export function createDamageNumbers(scene:Scene){
  ...
  const labels=Array.from({length:DAMAGE_NUMBER_COUNT},(_,i)=>{
    ...
    root.setEnabled(false);return {root,digits,life:0,damage:0,x:0,z:0,lane:0};
  });
  let index=0;
  function show(hit:HitEvent){
    const label=labels[index%DAMAGE_NUMBER_COUNT];label.life=DAMAGE_NUMBER_LIFE;label.damage=hit.damage;label.x=hit.x;label.z=hit.z;label.lane=(index++%3-1)*.12;
    label.root.position.set(hit.x,1.28,hit.z);label.root.scaling.setAll(1);label.root.setEnabled(true);
  ...
  function update(dt:number,motion:boolean){
    for(const label of labels)if(label.life>0){
      ...
      label.root.position.set(label.x,1.28+(motion?progress*.65:0),label.z);
